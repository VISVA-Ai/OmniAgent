# TUI Enhancement Plan — Omni Agent (`omni-cli`)
...
|---|---|---|---|
| `omni-cli` | Main binary: REPL loop, arg parsing, rendering, API bridge | ~3,600 | **Primary TUI surface** |
...
| 0.4 | **Create a `tui/` module** — Introduce `crates/omni-cli/src/tui/mod.rs` as the namespace for all new TUI components: `status_bar.rs`, `layout.rs`, `tool_panel.rs`, etc. | S |
