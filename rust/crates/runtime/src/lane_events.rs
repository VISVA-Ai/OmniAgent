...
pub struct LaneOwnership {
    ...
    pub owner: String,
    /// Workflow scope (e.g., omni-agent-dogfood, external-git-maintenance)
...
mod tests {
    ...
    fn lane_ownership_binding_includes_workflow_scope() {
        let ownership = LaneOwnership {
            owner: "omni-1".to_string(),
            workflow_scope: "omni-agent-dogfood".to_string(),
            watcher_action: WatcherAction::Act,
        };

        assert_eq!(ownership.owner, "omni-1");
        assert_eq!(ownership.workflow_scope, "omni-agent-dogfood");
