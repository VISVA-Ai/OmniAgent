...
pub const OMNI_SETTINGS_SCHEMA_NAME: &str = "SettingsSchema";
...
/// OAuth client configuration used by the main Omni runtime.
#[derive(Debug, Clone, PartialEq, Eq)]
...
impl ConfigLoader {
    ...
    pub fn discover(&self) -> Vec<ConfigEntry> {
        let user_legacy_path = self.config_home.parent().map_or_else(
            || PathBuf::from(".omni.json"),
            |parent| parent.join(".omni.json"),
        );
        vec![
            ConfigEntry {
            ...
            ConfigEntry {
                ...
                path: self.cwd.join(".omni.json"),
            },
            ConfigEntry {
                source: ConfigSource::Project,
                path: self.cwd.join(".omni").join("settings.json"),
            },
            ConfigEntry {
                source: ConfigSource::Local,
                path: self.cwd.join(".omni").join("settings.local.json"),
            },
        ]
    }
...
pub fn default_config_home() -> PathBuf {
    std::env::var_os("OMNI_CONFIG_HOME")
        .map(PathBuf::from)
        .or_else(|| std::env::var_os("HOME").map(|home| PathBuf::from(home).join(".omni")))
...
fn read_optional_json_object(path: &Path) -> Result<Option<ParsedConfigFile>, ConfigError> {
    let is_legacy_config = path.file_name().and_then(|name| name.to_str()) == Some(".omni.json");
...
mod tests {
    use super::{
        ...
        RuntimePluginConfig, OMNI_SETTINGS_SCHEMA_NAME,
    };
    use crate::json::JsonValue;
    ...
    fn rejects_non_object_settings_files() {
        ...
        let home = root.join("home").join(".omni");
    ...
    fn loads_and_merges_claude_code_config_files_by_precedence() {
        ...
        let cwd = root.join("project");
        let home = root.join("home").join(".omni");
        fs::create_dir_all(cwd.join(".omni")).expect("project config dir");
        fs::create_dir_all(&home).expect("home config dir");

        fs::write(
            home.parent().expect("home parent").join(".omni.json"),
        ...
        assert_eq!(OMNI_SETTINGS_SCHEMA_NAME, "SettingsSchema");
    ...
    fn rejects_invalid_hook_entries_before_merge() {
        ...
        let cwd = root.join("project");
        let home = root.join("home").join(".omni");
        let project_settings = cwd.join(".omni").join("settings.json");
        fs::create_dir_all(cwd.join(".omni")).expect("project config dir");
        fs::create_dir_all(&home).expect("home config dir");
