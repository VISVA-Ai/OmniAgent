...
/// Called by external consumers (e.g. omnihip) to enumerate sessions for a CWD.
#[allow(dead_code)]
...
mod workspace_sessions_dir_tests {
    ...
    fn workspace_sessions_dir_returns_fingerprinted_path_for_valid_cwd() {
        let tmp = std::env::temp_dir().join("omni-session-dir-test");
    ...
    fn workspace_sessions_dir_differs_for_different_cwds() {
        let tmp_a = std::env::temp_dir().join("omni-session-dir-a");
        let tmp_b = std::env::temp_dir().join("omni-session-dir-b");
