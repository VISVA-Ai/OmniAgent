...
impl PluginTool {
    ...
    pub fn execute(&self, input: &Value) -> Result<String, PluginError> {
        ...
        process
            ...
            .stdout(Stdio::piped())
            .stderr(Stdio::piped())
            .env("OMNID_PLUGIN_ID", &self.plugin_id)
            .env("OMNID_PLUGIN_NAME", &self.plugin_name)
            .env("OMNID_TOOL_NAME", &self.definition.name)
            .env("OMNID_TOOL_INPUT", &input_json);
        if let Some(root) = &self.root {
            process
                .current_dir(root)
                .env("OMNID_PLUGIN_ROOT", root.display().to_string());
        }
...
fn detect_claude_code_manifest_contract_gaps(
    ...
    for (field, detail) in [
        (
            "skills",
            "plugin manifest field `skills` uses the Claude Code plugin contract; `omni` does not load plugin-managed skills and instead discovers skills from local roots such as `.omni/skills`, `.omc/skills`, `.agents/skills`, `~/.omc/skills`, and `~/.claude/skills/omc-learned`.",
        ),
        (
            "mcpServers",
            "plugin manifest field `mcpServers` uses the Claude Code plugin contract; `omni` does not import MCP servers from plugin manifests.",
        ),
        (
            "agents",
            "plugin manifest field `agents` uses the Claude Code plugin contract; `omni` does not load plugin-managed agent markdown catalogs from plugin manifests.",
        ),
    ] {
    ...
    {
        errors.push(PluginManifestValidationError::UnsupportedManifestContract {
            detail: "plugin manifest field `commands` uses Claude Code-style directory globs; `omni` slash dispatch is still built-in and does not load plugin slash command markdown files.".to_string(),
        });
    }
    ...
    if let Some(hooks) = root.get("hooks").and_then(Value::as_object) {
        for hook_name in hooks.keys() {
            if !matches!(
                ...
                errors.push(PluginManifestValidationError::UnsupportedManifestContract {
                    detail: format!(
                        "plugin hook `{hook_name}` uses the Claude Code lifecycle contract; `omni` plugins currently support only PreToolUse, PostToolUse, and PostToolUseFailure."
                    ),
                });
            }
        }
...
/// Guards against concurrent modification of `OMNI_CONFIG_HOME`.
#[cfg(test)]
...
mod tests {
  ...
  "commands": [
    ...
    fn write_tool_plugin_with_name(root: &Path, name: &str, version: &str, tool_name: &str) {
        ...
        write_file(
            &script_path,
            "#!/bin/sh\nINPUT=$(cat)\nprintf '{\"plugin\":\"%s\",\"tool\":\"%s\",\"input\":%s}\\n' \"$OMNID_PLUGIN_ID\" \"$OMNID_TOOL_NAME\" \"$INPUT\"\n",
        );
        #[cfg(unix)]
        {
  ...
  "description": "Packaged manifest in install root"
    ...
    /// Regression test for ROADMAP #41: verify that `OMNI_CONFIG_HOME` isolation prevents
    /// host `~/.omni/plugins/` from bleeding into test runs.
    #[test]
    fn omni_config_home_isolation_prevents_host_plugin_leakage() {
        let _guard = env_guard();

        // Create a temp directory to act as our isolated OMNI_CONFIG_HOME
        let config_home = temp_dir("isolated-home");
        let bundled_root = temp_dir("isolated-bundled");

        // Set OMNI_CONFIG_HOME to our temp directory
  ...
  "description": "Test fixture plugin in isolated config home"
        ...
        // Create PluginManager with isolated bundled_root - it should use the temp config_home, not host ~/.omni/
        let mut config = PluginManagerConfig::new(&config_home);
        config.bundled_root = Some(bundled_root.clone());
        let manager = PluginManager::new(config);
        ...
        assert_eq!(
            installed.len(),
            1,
            "should only see the test fixture plugin, not host ~/.omni/plugins/"
        );
        assert_eq!(
