...
fn build_http_client() -> Result<Client, String> {
    Client::builder()
        ...
        .user_agent("omnid-rust-tools/0.1")
        .build()
        .map_err(|error| error.to_string())
}
...
fn build_search_url(query: &str) -> Result<reqwest::Url, String> {
    if let Ok(base) = std::env::var("OMNID_WEB_SEARCH_BASE_URL") {
...
fn todo_store_path() -> Result<std::path::PathBuf, String> {
    if let Ok(path) = std::env::var("OMNID_TODO_STORE") {
        return Ok(std::path::PathBuf::from(path));
    }
    let cwd = std::env::current_dir().map_err(|error| error.to_string())?;
    Ok(cwd.join(".omnid-todos.json"))
}
...
fn skill_lookup_roots() -> Vec<SkillLookupRoot> {
    ...
    if let Ok(omni_config_home) = std::env::var("OMNI_CONFIG_HOME") {
        push_prefixed_skill_lookup_roots(&mut roots, std::path::Path::new(&omni_config_home));
    }
    ...
    push_skill_lookup_root(
        &mut roots,
        std::path::PathBuf::from("/home/bellman/.omni/skills"),
        SkillLookupOrigin::SkillsDir,
    );
    push_skill_lookup_root(
        &mut roots,
...
fn push_project_skill_lookup_roots(roots: &mut Vec<SkillLookupRoot>, cwd: &std::path::Path) {
    for ancestor in cwd.ancestors() {
        ...
        push_prefixed_skill_lookup_roots(roots, &ancestor.join(".omni"));
...
fn push_home_skill_lookup_roots(roots: &mut Vec<SkillLookupRoot>, home: &std::path::Path) {
    ...
    push_prefixed_skill_lookup_roots(roots, &home.join(".omni"));
...
fn spawn_agent_job(job: AgentJob) -> Result<(), String> {
    let thread_name = format!("omnid-agent-{}", job.manifest.agent_id);
...
fn agent_store_dir() -> Result<std::path::PathBuf, String> {
    if let Ok(path) = std::env::var("OMNID_AGENT_STORE") {
        return Ok(std::path::PathBuf::from(path));
    }
    let cwd = std::env::current_dir().map_err(|error| error.to_string())?;
    if let Some(workspace_root) = cwd.ancestors().nth(2) {
        return Ok(workspace_root.join(".omnid-agents"));
    }
    Ok(cwd.join(".omnid-agents"))
}
...
fn normalize_subagent_type(subagent_type: Option<&str>) -> String {
    ...
    match canonical_tool_token(trimmed).as_str() {
        ...
        "omniguide" | "omniguideagent" | "guide" => String::from("omni-guide"),
...
fn config_file_for_scope(scope: ConfigScope) -> Result<PathBuf, String> {
    ...
    Ok(match scope {
        ...
        ConfigScope::Settings => cwd.join(".omni").join("settings.local.json"),
    })
}

fn config_home_dir() -> Result<PathBuf, String> {
    if let Ok(path) = std::env::var("OMNI_CONFIG_HOME") {
        ...
        .map_err(|_| {
            String::from(
                "HOME is not set (on Windows, set USERPROFILE or HOME, \
                 or use OMNI_CONFIG_HOME to point directly at the config directory)",
            )
        })?;
    Ok(PathBuf::from(home).join(".omni"))
}
...
mod tests {
    ...
    fn temp_path(name: &str) -> PathBuf {
        let unique = std::time::SystemTime::now()
            ...
            .expect("time")
            .as_nanos();
        std::env::temp_dir().join(format!("omnid-tools-{unique}-{name}"))
    }
    ...
    fn worker_create_merges_config_trusted_roots_without_per_call_override() {
        use std::fs;
        // Write a .omni/settings.json in a temp dir with trustedRoots
        let worktree = temp_path("config-trust-worktree");
        let omni_dir = worktree.join(".omni");
        fs::create_dir_all(&omni_dir).expect("create .omni dir");
        ...
        fs::write(omni_dir.join("settings.json"), settings).expect("write settings");
    ...
    fn recovery_loop_state_file_reflects_transitions() {
        // End-to-end proof: .omni/worker-state.json reflects every transition
        ...
        let state_path = worktree.join(".omni").join("worker-state.json");
    ...
    fn stall_detect_and_resolve_trust_end_to_end() {
        ...
        // 3. Omnihip calls WorkerResolveTrust to unblock
    ...
    fn stall_detect_and_restart_recovery_end_to_end() {
        // Worker stalls at trust_required, omnihip restarts instead of resolving
        let created = execute_tool(
            "WorkerCreate",
    ...
    fn web_search_extracts_and_filters_results() {
        ...
        // sets OMNID_WEB_SEARCH_BASE_URL. Without the lock, parallel test
        ...
        std::env::remove_var("OMNID_WEB_SEARCH_BASE_URL");
    ...
    fn web_search_handles_generic_links_and_invalid_base_url() {
        ...
        std::env::set_var("OMNID_WEB_SEARCH_BASE_URL", "://bad-base-url");
    ...
    fn todo_write_persists_and_returns_previous_state() {
        ...
        std::env::set_var("OMNID_TODO_STORE", &path);
        ...
        .expect("TodoWrite should succeed");
        std::env::remove_var("OMNID_TODO_STORE");
    ...
    fn skill_resolves_project_local_skills_and_legacy_commands() {
        ...
        let skill_dir = root.join(".omni").join("skills").join("plan");
        let command_dir = root.join(".omni").join("commands");
        fs::create_dir_all(&skill_dir).expect("skill dir should exist");
        ...
        assert!(skill_output["path"]
            .as_str()
            .expect("path")
            .ends_with(".omni/skills/plan/SKILL.md"));
        ...
        assert!(command_output["path"]
            .as_str()
            .expect("path")
            .ends_with(".omni/commands/handoff.md"));
    ...
    fn agent_persists_handoff_metadata() {
        ...
        std::env::set_var("OMNID_AGENT_STORE", &dir);
        ...
        std::env::remove_var("OMNID_AGENT_STORE");
  ...
  "nbformat_minor": 5
    ...
    fn config_reads_and_writes_supported_values() {
        ...
        std::fs::create_dir_all(home.join(".omni")).expect("home dir");
        std::fs::create_dir_all(cwd.join(".omni")).expect("cwd dir");
        std::fs::write(
            home.join(".omni").join("settings.json"),
            r#"{"verbose":false}"#,
        )
    ...
    fn enter_and_exit_plan_mode_round_trip_existing_local_override() {
        ...
        std::fs::create_dir_all(cwd.join(".omni")).expect("cwd dir");
        std::fs::write(
            cwd.join(".omni").join("settings.local.json"),
        ...
        let local_settings = std::fs::read_to_string(cwd.join(".omni").join("settings.local.json"))
            .expect("local settings after enter");
        assert!(local_settings.contains(r#""defaultMode": "plan""#));
        let state =
            std::fs::read_to_string(cwd.join(".omni").join("tool-state").join("plan-mode.json"))
...
"#,
    ...
    fn run_task_packet_creates_packet_backed_task() {
        ...
        let result = run_task_packet(TaskPacket {
            ...
            repo: "omni-agent-parity".to_string(),
        ...
        assert_eq!(output["task_packet"]["repo"], "omni-agent-parity");
        assert_eq!(
