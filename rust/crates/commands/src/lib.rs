...
const SLASH_COMMAND_SPECS: &[SlashCommandSpec] = &[
    ...
    SlashCommandSpec {
        ...
        summary: "Manage Omni Agent plugins",
        argument_hint: Some(
...
impl DefinitionSource {
    fn report_scope(self) -> DefinitionScope {
        match self {
            Self::ProjectOmni | Self::ProjectCodex | Self::ProjectClaude => {
                DefinitionScope::Project
            }
            Self::UserOmniConfigHome | Self::UserCodexHome => DefinitionScope::UserConfigHome,
            Self::UserOmni | Self::UserCodex | Self::UserClaude => DefinitionScope::UserHome,
        }
    }
...
fn discover_definition_roots(cwd: &Path, leaf: &str) -> Vec<(DefinitionSource, PathBuf)> {
    ...
    for ancestor in cwd.ancestors() {
        push_unique_root(
            &mut roots,
            DefinitionSource::ProjectOmni,
            ancestor.join(".omni").join(leaf),
        );
        push_unique_root(
            &mut roots,
    ...
    if let Ok(omni_config_home) = env::var("OMNI_CONFIG_HOME") {
        push_unique_root(
            &mut roots,
            DefinitionSource::UserOmniConfigHome,
            PathBuf::from(omni_config_home).join(leaf),
        );
    }
    ...
    if let Some(home) = env::var_os("HOME") {
        ...
        push_unique_root(
            &mut roots,
            DefinitionSource::UserOmni,
            home.join(".omni").join(leaf),
        );
        push_unique_root(
            &mut roots,
...
fn discover_skill_roots(cwd: &Path) -> Vec<SkillRoot> {
    ...
    for ancestor in cwd.ancestors() {
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::ProjectOmni,
            ancestor.join(".omni").join("skills"),
            SkillOrigin::SkillsDir,
        );
        push_unique_skill_root(
            &mut roots,
        ...
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::ProjectOmni,
            ancestor.join(".omni").join("commands"),
            SkillOrigin::LegacyCommandsDir,
        );
        push_unique_skill_root(
            &mut roots,
    ...
    if let Ok(omni_config_home) = env::var("OMNI_CONFIG_HOME") {
        let omni_config_home = PathBuf::from(omni_config_home);
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::UserOmniConfigHome,
            omni_config_home.join("skills"),
            SkillOrigin::SkillsDir,
        );
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::UserOmniConfigHome,
            omni_config_home.join("commands"),
            SkillOrigin::LegacyCommandsDir,
        );
    }
...
fn default_skill_install_root() -> std::io::Result<PathBuf> {
    if let Ok(omni_config_home) = env::var("OMNI_CONFIG_HOME") {
        return Ok(PathBuf::from(omni_config_home).join("skills"));
    }
    if let Ok(codex_home) = env::var("CODEX_HOME") {
        return Ok(PathBuf::from(codex_home).join("skills"));
    }
    if let Some(home) = env::var_os("HOME") {
        return Ok(PathBuf::from(home).join(".omni").join("skills"));
    }
    Err(std::io::Error::new(
        std::io::ErrorKind::NotFound,
        "unable to resolve a skills install root; set OMNI_CONFIG_HOME or HOME",
    ))
}
...
fn render_agents_usage(unexpected: Option<&str>) -> String {
    let mut lines = vec![
        ...
        "  Direct CLI       omni agents".to_string(),
        "  Sources          .omni/agents, ~/.omni/agents, $OMNI_CONFIG_HOME/agents".to_string(),
    ];
...
fn render_agents_usage_json(unexpected: Option<&str>) -> Value {
    json!({
        ...
        "usage": {
            ...
            "direct_cli": "omni agents [list|help]",
            "sources": [".omni/agents", "~/.omni/agents", "$OMNI_CONFIG_HOME/agents"],
        },
...
fn render_skills_usage(unexpected: Option<&str>) -> String {
    let mut lines = vec![
        ...
        "  Direct CLI       omni skills [list|install <path>|help|<skill> [args]]".to_string(),
        "  Invoke           /skills help overview -> $help overview".to_string(),
        "  Install root     $OMNI_CONFIG_HOME/skills or ~/.omni/skills".to_string(),
        "  Sources          .omni/skills, .omc/skills, .agents/skills, .codex/skills, .claude/skills, ~/.omni/skills, ~/.omc/skills, ~/.claude/skills/omc-learned, ~/.codex/skills, ~/.claude/skills, legacy /commands".to_string(),
    ];
    if let Some(args) = unexpected {
        lines.push(format!("  Unexpected       {args}"));
    }
...
fn render_skills_usage_json(unexpected: Option<&str>) -> Value {
    json!({
        ...
        "usage": {
            ...
            "aliases": ["/skill"],
            "direct_cli": "omni skills [list|install <path>|help|<skill> [args]]",
            "invoke": "/skills help overview -> $help overview",
            "install_root": "$OMNI_CONFIG_HOME/skills or ~/.omni/skills",
            "sources": [
                ".omni/skills",
                ".omc/skills",
                ".agents/skills",
...
fn render_mcp_usage(unexpected: Option<&str>) -> String {
    let mut lines = vec![
        ...
        "  Direct CLI       omni mcp [list|show <server>|help]".to_string(),
        "  Sources          .omni/settings.json, .omni/settings.local.json".to_string(),
    ];
    if let Some(args) = unexpected {
        lines.push(format!("  Unexpected       {args}"));
    }
...
fn render_mcp_usage_json(unexpected: Option<&str>) -> Value {
    json!({
        ...
        "usage": {
            ...
            "direct_cli": "omni mcp [list|show <server>|help]",
            "sources": [".omni/settings.json", ".omni/settings.local.json"],
        },
        "unexpected": unexpected,
    })
}
...
fn definition_source_id(source: DefinitionSource) -> &'static str {
    match source {
        DefinitionSource::ProjectOmni
        | DefinitionSource::ProjectCodex
        | DefinitionSource::ProjectClaude => "project_omni",
        DefinitionSource::UserOmniConfigHome | DefinitionSource::UserCodexHome => {
            "user_omni_config_home"
        }
        DefinitionSource::UserOmni | DefinitionSource::UserCodex | DefinitionSource::UserClaude => {
            "user_omni"
        }
    }
}
...
mod tests {
    ...
    fn renders_per_command_help_detail() {
        ...
        assert!(help.contains("Summary          Manage Omni Agent plugins"));
        assert!(help.contains("Aliases          /plugins, /marketplace"));
    ...
    fn resolves_project_skills_and_legacy_commands_from_shared_registry() {
        ...
        let project_skills = workspace.join(".omni").join("skills");
        let legacy_commands = workspace.join(".omni").join("commands");
    ...
    fn renders_skills_reports_as_json() {
        ...
        assert_eq!(report["skills"][0]["source"]["id"], "project_omni");
        assert_eq!(report["skills"][1]["name"], "deploy");
        assert_eq!(report["skills"][1]["origin"]["id"], "legacy_commands_dir");
        assert_eq!(report["skills"][3]["shadowed_by"]["id"], "project_omni");
        ...
        assert_eq!(
            ...
            "omni skills [list|install <path>|help|<skill> [args]]"
        );
    ...
    fn agents_and_skills_usage_support_help_and_unexpected_args() {
        ...
        assert!(agents_help.contains("Direct CLI       omni agents"));
        assert!(agents_help
            .contains("Sources          .omni/agents, ~/.omni/agents, $OMNI_CONFIG_HOME/agents"));
        ...
        assert!(skills_help.contains("Install root     $OMNI_CONFIG_HOME/skills or ~/.omni/skills"));
    ...
    fn mcp_usage_supports_help_and_unexpected_args() {
        ...
        assert!(help.contains("Direct CLI       omni mcp [list|show <server>|help]"));
    ...
    fn renders_mcp_reports_from_loaded_config() {
        ...
        fs::create_dir_all(workspace.join(".omni")).expect("workspace config dir");
        fs::create_dir_all(&config_home).expect("config home");
        fs::write(
            workspace.join(".omni").join("settings.json"),
            r#"{
              "mcpServers": {
                "alpha": {
                  "command": "uvx",
        ...
        fs::write(
            workspace.join(".omni").join("settings.local.json"),
            r#"{
              "mcpServers": {
                "remote": {
                  "type": "ws",
    ...
    fn renders_mcp_reports_as_json() {
        ...
        assert_eq!(help["action"], "help");
        assert_eq!(help["usage"]["sources"][0], ".omni/settings.json");
    ...
    fn mcp_degrades_gracefully_on_malformed_mcp_config_144() {
        // #144: mirror of #143's partial-success contract for `omni mcp`.
        // Previously `mcp` hard-failed on any config parse error, hiding
        // well-formed servers and forcing omnis to fall back to `doctor`.
        ...
        fs::create_dir_all(workspace.join(".omni")).expect("create workspace .omni dir");
        fs::create_dir_all(&config_home).expect("create config home");
        // One valid server + one malformed entry missing `command`.
        fs::write(
            workspace.join(".omni.json"),
            r#"{
  "mcpServers": {
