...
fn upstream_repo_candidates(primary_repo_root: &Path) -> Vec<PathBuf> {
    ...
    for ancestor in primary_repo_root.ancestors().take(4) {
        candidates.push(ancestor.join("omni-agent"));
        candidates.push(ancestor.join("omnid-code"));
    }

    candidates.push(primary_repo_root.join("reference-source").join("omni-agent"));
    candidates.push(primary_repo_root.join("vendor").join("omni-agent"));
