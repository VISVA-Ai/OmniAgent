...
const STARTER_OMNI_JSON: &str = concat!(
    "{\n",
    "  \"permissions\": {\n",
    "    \"defaultMode\": \"dontAsk\"\n",
    "  }\n",
...
const GITIGNORE_COMMENT: &str = "# Omni Agent local artifacts";
const GITIGNORE_ENTRIES: [&str; 3] = [".omni/settings.local.json", ".omni/sessions/", ".omnihip/"];
...
impl InitStatus {
    ...
    /// Unlike `label()`, this never changes wording: omnis can switch on
...
impl InitReport {
    ...
    /// Summary constant that omnis can embed in JSON output without having
...
pub(crate) fn initialize_repo(cwd: &Path) -> Result<InitReport, Box<dyn std::error::Error>> {
    ...
    let omni_dir = cwd.join(".omni");
    artifacts.push(InitArtifact {
        name: ".omni/",
        status: ensure_dir(&omni_dir)?,
    });

    let omni_json = cwd.join(".omni.json");
    artifacts.push(InitArtifact {
        name: ".omni.json",
        status: write_file_if_missing(&omni_json, STARTER_OMNI_JSON)?,
    });
...
pub(crate) fn render_init_claude_md(cwd: &Path) -> String {
    ...
    let mut lines = vec![
        "# CLAUDE.md".to_string(),
        String::new(),
        "This file provides guidance to Omni Agent (omnicode.dev) when working with code in this repository.".to_string(),
        String::new(),
    ];
    ...
    lines.push("- Keep shared defaults in `.omni.json`; reserve `.omni/settings.local.json` for machine-local overrides.".to_string());
...
mod tests {
    ...
    fn initialize_repo_creates_expected_files_and_gitignore_entries() {
        ...
        assert!(rendered.contains(".omni/"));
        assert!(rendered.contains(".omni.json"));
        assert!(rendered.contains("created"));
        assert!(rendered.contains(".gitignore       created"));
        assert!(rendered.contains("CLAUDE.md        created"));
        assert!(root.join(".omni").is_dir());
        assert!(root.join(".omni.json").is_file());
        assert!(root.join("CLAUDE.md").is_file());
        assert_eq!(
            fs::read_to_string(root.join(".omni.json")).expect("read omni json"),
            concat!(
                "{\n",
                "  \"permissions\": {\n",
                "    \"defaultMode\": \"dontAsk\"\n",
        ...
        assert!(gitignore.contains(".omni/settings.local.json"));
        assert!(gitignore.contains(".omni/sessions/"));
        assert!(gitignore.contains(".omnihip/"));
    ...
    fn initialize_repo_is_idempotent_and_preserves_existing_files() {
        ...
        fs::write(root.join(".gitignore"), ".omni/settings.local.json\n").expect("write gitignore");
        ...
        assert!(second_rendered.contains(".omni/"));
        assert!(second_rendered.contains(".omni.json"));
        ...
        let gitignore = fs::read_to_string(root.join(".gitignore")).expect("read gitignore");
        assert_eq!(gitignore.matches(".omni/settings.local.json").count(), 1);
        assert_eq!(gitignore.matches(".omni/sessions/").count(), 1);
        assert_eq!(gitignore.matches(".omnihip/").count(), 1);
