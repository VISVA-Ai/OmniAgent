...
/// #148: Model provenance for `omni status` JSON/text output. Records where
/// the resolved model string came from so omnis don't have to re-read argv
...
enum ModelSource {
    ...
    Env,
    /// `model` key in `.omni.json` / `.omni/settings.json` (when neither
    /// flag nor env set it).
    Config,
...
const OFFICIAL_REPO_URL: &str = "https://github.com/visva/omni-agent";
const OFFICIAL_REPO_SLUG: &str = "visva/omni-agent";
const DEPRECATED_INSTALL_COMMAND: &str = "cargo install omni-agent";
...
fn main() {
    if let Err(error) = run() {
        ...
        if json_output {
            // #77: classify error by prefix so downstream omnis can route without
...
enum CliAction {
    ...
    // #146: `omni config` and `omni diff` are pure-local read-only
...
enum LocalHelpTopic {
    ...
    // `omni <subcommand> --help` has one consistent contract.
    Init,
    State,
    Export,
    Version,
...
fn parse_args(args: &[String]) -> Result<CliAction, String> {
    ...
    while index < args.len() {
        match args[index].as_str() {
                    ...
                    ) =>
            {
                ...
                // the arg to the API (e.g. `omni prompt --help`) should show
            ...
            "-p" => {
                // Omni Agent compat: -p "prompt" = one-shot prompt
            ...
            "--print" => {
                // Omni Agent compat: --print makes output non-interactive
    ...
    match rest[0].as_str() {
        ...
        // top-level parser arm produced CliAction::Plugins. That made `omni
        // plugins` (and `omni plugins --help`, `omni plugins list`, ...)
        ...
        "plugins" => {
            ...
            if tail.len() > 2 {
                return Err(format!(
                    "unexpected extra arguments after `omni plugins {}`: {}",
                    tail[..2].join(" "),
                    tail[2..].join(" ")
                ));
            }
        ...
        // `.omni.json` + `.omni/settings.json` from disk, no network, no
        // state mutation). Previously callers had to spin up a session with
        // `omni --resume SESSION.jsonl /config` to see their own config,
        ...
        "config" => {
            ...
            if tail.len() > 1 {
                return Err(format!(
                    "unexpected extra arguments after `omni config {}`: {}",
                    tail[0],
                    tail[1..].join(" ")
                ));
            }
        ...
        "diff" => {
            if rest.len() > 1 {
                return Err(format!(
                    "unexpected extra arguments after `omni diff`: {}",
                    rest[1..].join(" ")
                ));
            }
        ...
        other => {
            if rest.len() == 1 && looks_like_subcommand_typo(other) {
                if let Some(suggestions) = suggest_similar_subcommand(other) {
                    ...
                    message.push_str(
                        "\nRun `omni --help` for the full list. If you meant to send a prompt literally, use `omni prompt <text>`.",
                    );
                    return Err(message);
                }
            }
            ...
            // `omni ""`, `omni "   "`, and `omni "" ""` silently route to
            ...
            if joined.trim().is_empty() {
                return Err(
                    "empty prompt: provide a subcommand (run `omni --help`) or a non-empty prompt string"
                        .to_string(),
                );
            }
            Ok(CliAction::Prompt {
...
fn bare_slash_command_guidance(command_name: &str) -> Option<String> {
    ...
    let guidance = if slash_command.resume_supported {
        format!(
            "`omni {command_name}` is a slash command. Use `omni --resume SESSION.jsonl /{command_name}` or start `omni` and run `/{command_name}`."
        )
    } else {
        format!(
            "`omni {command_name}` is a slash command. Start `omni` and run `/{command_name}` inside the REPL."
        )
    };
    Some(guidance)
}
...
fn removed_auth_surface_error(command_name: &str) -> String {
    format!(
        "`omni {command_name}` has been removed. Set ANTHROPIC_API_KEY or ANTHROPIC_AUTH_TOKEN instead."
    )
}
...
fn parse_acp_args(args: &[String], output_format: CliOutputFormat) -> Result<CliAction, String> {
    match args {
        ...
        _ => Err(String::from(
            "unsupported ACP invocation. Use `omni acp`, `omni acp serve`, `omni --acp`, or `omni -acp`.",
        )),
    }
}
...
fn parse_direct_slash_cli_action(
    ...
    match SlashCommand::parse(&raw) {
        ...
        Ok(Some(command)) => Err({
            let _ = command;
            format!(
                "slash command {command_name} is interactive-only. Start `omni` and run it there, or use `omni --resume SESSION.jsonl {command_name}` / `omni --resume {latest} {command_name}` when the command is marked [resume] in /help.",
                command_name = rest[0],
...
fn format_unknown_direct_slash_command(name: &str) -> String {
    ...
    message.push_str("\nRun `omni --help` for CLI usage, or start `omni` and use /help.");
    message
}
...
fn omc_compatibility_note_for_unknown_slash_command(name: &str) -> Option<&'static str> {
    name.starts_with("oh-my-claudecode:")
        .then_some(
            "Compatibility note: `/oh-my-claudecode:*` is a Claude Code/OMC plugin command. `omni` does not yet load plugin slash commands, Claude statusline stdin, or OMC session hooks.",
        )
}
...
/// Starts a minimal Model Context Protocol server that exposes omni's
...
/// Read `.omni/worker-state.json` from the current working directory and print it.
/// This is the file-based worker observability surface: `push_event()` in `worker_boot.rs`
/// atomically writes state transitions here so external observers (omnihip, orchestrators)
...
fn run_worker_state(output_format: CliOutputFormat) -> Result<(), Box<dyn std::error::Error>> {
    let cwd = env::current_dir()?;
    let state_path = cwd.join(".omni").join("worker-state.json");
    if !state_path.exists() {
        ...
        // no `omni worker` subcommand), so omnis/CI had no discoverable path
        ...
        //     Run:   omni               # start the REPL (writes state on first turn)
        //     Or:    omni prompt <text> # run one non-interactive turn
        //     Then rerun: omni state [--output-format json]
        return Err(format!(
...
fn check_auth_health() -> DiagnosticCheck {
    ...
    match load_oauth_credentials() {
        ...
        .with_details(vec![
            ...
            "Suggested action  set ANTHROPIC_API_KEY or ANTHROPIC_AUTH_TOKEN; `omni login` is removed"
                .to_string(),
        ])
...
fn check_install_source_health() -> DiagnosticCheck {
    ...
    .with_details(vec![
        ...
        "Recommended path  build from this repo or use the upstream binary documented in README.md"
            .to_string(),
        format!(
            "Deprecated crate  `{DEPRECATED_INSTALL_COMMAND}` installs a deprecated stub and does not provide the `omni` binary"
        )
            .to_string(),
    ])
    .with_data(Map::from_iter([
...
const DUMP_MANIFESTS_OVERRIDE_HINT: &str =
    "Hint: set CLAUDE_CODE_UPSTREAM=/path/to/upstream or pass `omni dump-manifests --manifests-dir /path/to/upstream`.";
...
struct StatusContext {
    ...
    /// #143: when `.omni.json` (or another loaded config file) fails to parse,
    ...
    /// `status: "degraded"` so omnis can distinguish "status ran but config
...
fn render_resume_usage() -> String {
  ...
  Auto-save        .omni/sessions/<session-id>.{PRIMARY_SESSION_EXTENSION}
    ...
    match command {
        ...
        SlashCommand::Skills { args } => {
            if let SkillSlashDispatch::Invoke(_) = classify_skills_slash_command(args.as_deref()) {
                return Err(
                    "resumed /skills invocations are interactive-only; start `omni` and run `/skills <skill>` in the REPL".into(),
                );
            }
            let cwd = env::current_dir()?;
            Ok(ResumeCommandOutcome {
...
fn enforce_broad_cwd_policy(
    ...
    if is_interactive {
        ...
        eprintln!(
            "Warning: omni is running from a very broad directory ({}).\n\
             The agent can read and search everything under this path.\n\
             Consider running from inside your project: cd /path/to/project && omni",
            cwd.display()
        );
        ...
        let message = format!(
            "omni is running from a very broad directory ({}). \
             The agent can read and search everything under this path. \
             Use --allow-broad-cwd to proceed anyway, \
             or run from inside your project: cd /path/to/project && omni",
            cwd.display()
        );
        match output_format {
            CliOutputFormat::Json => {
...
impl LiveCli {
 ...
 ██║   ██║██║╚██╔╝██║██║╚████║██║\n\
 ╚██████╔╝██║ ╚═╝ ██║██║ ╚███║██║\n\
  ╚═════╝ ╚═╝     ╚═╝╚═╝  ╚══╝╚═╝\x1b[0m \x1b[38;5;208mAgent\x1b[0m 🤖\n\n\
  \x1b[2mModel\x1b[0m            {}\n\
  ...
  Type \x1b[1m/help\x1b[0m for commands · \x1b[1m/status\x1b[0m for live context · \x1b[2m/resume latest\x1b[0m jumps back to the newest session · \x1b[1m/diff\x1b[0m then \x1b[1m/commit\x1b[0m to ship · \x1b[2mTab\x1b[0m for workflow completions · \x1b[2mShift+Enter\x1b[0m for newline",
    ...
    fn print_mcp(
        ...
        // `omni mcp serve` starts a stdio MCP server exposing omni's built-in
...
fn render_repl_help() -> String {
    [
        ...
        "  Auto-save            .omni/sessions/<session-id>.jsonl".to_string(),
...
fn status_json_value(
    ...
    // #143: top-level `status` marker so omnis can distinguish
    ...
    json!({
        ...
        "workspace": {
            ...
            "session_id": context.session_path.as_ref().and_then(|path| {
                ...
                // .omni/sessions/. Extract the stem (drop the .jsonl extension).
...
fn status_context(
    ...
    // `omni doctor` already does this; `omni status` now matches that contract
    ...
    let (loaded_config_files, sandbox_status, config_load_error) = match loader.load() {
        ...
        Err(err) => (
            0,
            // Fall back to defaults for sandbox resolution so omnis still see
...
fn format_status_report(
    ...
    if let Some(err) = context.config_load_error.as_deref() {
        blocks.push(format!(
            "Config load error\n  Status           fail\n  Summary          runtime config failed to load; reporting partial status\n  Details          {err}\n  Hint             `omni doctor` classifies config parse errors; fix the listed field and rerun"
        ));
    }
...
fn render_help_topic(topic: LocalHelpTopic) -> String {
  ...
  Usage            omni status [--output-format <format>]
  Purpose          show the local workspace snapshot without entering the REPL
  Output           model, permissions, git state, config files, and sandbox status
  Formats          text (default), json
  Related          /status · omni --resume latest /status"
            .to_string(),
        LocalHelpTopic::Sandbox => "Sandbox
  Usage            omni sandbox [--output-format <format>]
  Purpose          inspect the resolved sandbox and isolation state for the current directory
  Output           namespace, network, filesystem, and fallback details
  Formats          text (default), json
  Related          /sandbox · omni status"
            .to_string(),
        LocalHelpTopic::Doctor => "Doctor
  Usage            omni doctor [--output-format <format>]
  Purpose          diagnose local auth, config, workspace, sandbox, and build metadata
  Output           local-only health report; no provider request or session resume required
  Formats          text (default), json
  Related          /doctor · omni --resume latest /doctor"
            .to_string(),
        LocalHelpTopic::Acp => "ACP / Zed
  Usage            omni acp [serve] [--output-format <format>]
  Aliases          omni --acp · omni -acp
  ...
  Formats          text (default), json
  Related          ROADMAP #64a (discoverability) · ROADMAP #76 (real ACP support) · omni --help"
            .to_string(),
        LocalHelpTopic::Init => "Init
  Usage            omni init [--output-format <format>]
  Purpose          create .omni/, .omni.json, .gitignore, and CLAUDE.md in the current project
  Output           list of created vs. skipped files (idempotent: safe to re-run)
  Formats          text (default), json
  Related          omni status · omni doctor"
            .to_string(),
        LocalHelpTopic::State => "State
  Usage            omni state [--output-format <format>]
  Purpose          read .omni/worker-state.json written by the interactive REPL or a one-shot prompt
  Output           worker id, model, permissions, session reference (text or json)
  Formats          text (default), json
  Produces state   `omni` (interactive REPL) or `omni prompt <text>` (one non-interactive turn)
  Observes state   `omni state` reads; omnihip/CI may poll this file without HTTP
  Exit codes       0 if state file exists and parses; 1 with actionable hint otherwise
  Related          omni status · ROADMAP #139 (this worker-concept contract)"
            .to_string(),
        LocalHelpTopic::Export => "Export
  Usage            omni export [--session <id|latest>] [--output <path>] [--output-format <format>]
  Purpose          serialize a managed session to JSON for review, transfer, or archival
  Defaults         --session latest (most recent managed session in .omni/sessions/)
  Formats          text (default), json
  Related          /session list · omni --resume latest"
            .to_string(),
        LocalHelpTopic::Version => "Version
  Usage            omni version [--output-format <format>]
  Aliases          omni --version · omni -V
  Purpose          print the omni CLI version and build metadata
  Formats          text (default), json
  Related          omni doctor (full build/auth/config diagnostic)"
            .to_string(),
        LocalHelpTopic::SystemPrompt => "System Prompt
  Usage            omni system-prompt [--cwd <path>] [--date YYYY-MM-DD] [--output-format <format>]
  Purpose          render the resolved system prompt that `omni` would send for the given cwd + date
  Options          --cwd overrides the workspace dir · --date injects a deterministic date stamp
  Formats          text (default), json
  Related          omni doctor · omni dump-manifests"
            .to_string(),
        LocalHelpTopic::DumpManifests => "Dump Manifests
  Usage            omni dump-manifests [--manifests-dir <path>] [--output-format <format>]
  ...
  Formats          text (default), json
  Related          omni skills · omni agents · omni doctor"
            .to_string(),
        LocalHelpTopic::BootstrapPlan => "Bootstrap Plan
  Usage            omni bootstrap-plan [--output-format <format>]
  ...
  Formats          text (default), json
  Related          omni doctor · omni status"
            .to_string(),
    }
}
...
fn print_acp_status(output_format: CliOutputFormat) -> Result<(), Box<dyn std::error::Error>> {
    let message = "ACP/Zed editor integration is not implemented in omni-agent yet. `omni acp serve` is only a discoverability alias today; it does not launch a daemon or Zed-specific protocol endpoint. Use the normal terminal surfaces for now and track ROADMAP #76 for real ACP support.";
    match output_format {
        CliOutputFormat::Text => {
            println!(
                "ACP / Zed\n  Status           discoverability only\n  Launch           `omni acp serve` / `omni --acp` / `omni -acp` report status only; no editor daemon is available yet\n  Today            use `omni prompt`, the REPL, or `omni doctor` for local verification\n  Tracking         ROADMAP #76\n  Message          {message}"
            );
        }
        CliOutputFormat::Json => {
            println!(
...
/// string so omnis can detect per-artifact state without substring matching.
...
impl AnthropicRuntimeClient {
    fn new(
        ...
        // omni at its fake Anthropic endpoint. We also attach a
...
fn format_context_window_blocked_error(session_id: &str, error: &api::ApiError) -> String {
    ...
    lines.push(format!(
        "  Resume compact   omni --resume {session_id} /compact"
    ));
...
const READ_DISPLAY_MAX_LINES: usize = 5;
const READ_DISPLAY_MAX_CHARS: usize = 500;
const TOOL_OUTPUT_DISPLAY_MAX_LINES: usize = 10;
const TOOL_OUTPUT_DISPLAY_MAX_CHARS: usize = 1_000;
...
fn print_help_to(out: &mut impl Write) -> io::Result<()> {
    writeln!(out, "omni v{VERSION}")?;
    writeln!(out)?;
    writeln!(out, "Usage:")?;
    writeln!(
        out,
        "  omni [--model MODEL] [--allowedTools TOOL[,TOOL...]]"
    )?;
    writeln!(out, "      Start the interactive REPL")?;
    writeln!(
        out,
        "  omni [--model MODEL] [--output-format text|json] prompt TEXT"
    )?;
    writeln!(out, "      Send one prompt and exit")?;
    writeln!(
        out,
        "  omni [--model MODEL] [--output-format text|json] TEXT"
    )?;
    writeln!(out, "      Shorthand non-interactive prompt mode")?;
    writeln!(
        out,
        "  omni --resume [SESSION.jsonl|session-id|latest] [/status] [/compact] [...]"
    )?;
    writeln!(
        out,
        "      Inspect or maintain a saved session without entering the REPL"
    )?;
    writeln!(out, "  omni help")?;
    writeln!(out, "      Alias for --help")?;
    writeln!(out, "  omni version")?;
    writeln!(out, "      Alias for --version")?;
    writeln!(out, "  omni status")?;
    writeln!(
        out,
        "      Show the current local workspace status snapshot"
    )?;
    writeln!(out, "  omni sandbox")?;
    writeln!(out, "      Show the current sandbox isolation snapshot")?;
    writeln!(out, "  omni doctor")?;
    writeln!(
        out,
        "      Diagnose local auth, config, workspace, and sandbox health"
    )?;
    writeln!(out, "  omni acp [serve]")?;
    writeln!(
        out,
    ...
    writeln!(out, "  omni dump-manifests [--manifests-dir PATH]")?;
    writeln!(out, "  omni bootstrap-plan")?;
    writeln!(out, "  omni agents")?;
    writeln!(out, "  omni mcp")?;
    writeln!(out, "  omni skills")?;
    writeln!(out, "  omni system-prompt [--cwd PATH] [--date YYYY-MM-DD]")?;
    writeln!(out, "  omni init")?;
    writeln!(
        out,
        "  omni export [PATH] [--session SESSION] [--output PATH]"
    )?;
    writeln!(
        out,
    ...
    writeln!(
        out,
        "  REPL turns auto-save to .omni/sessions/<session-id>.{PRIMARY_SESSION_EXTENSION}"
    )?;
    writeln!(
        out,
    ...
    writeln!(out, "  omni --model claude-opus \"summarize this repo\"")?;
    writeln!(
        out,
        "  omni --output-format json prompt \"explain src/main.rs\""
    )?;
    writeln!(out, "  omni --compact \"summarize Cargo.toml\" | wc -l")?;
    writeln!(
        out,
        "  omni --allowedTools read,glob \"summarize Cargo.toml\""
    )?;
    writeln!(out, "  omni --resume {LATEST_SESSION_REFERENCE}")?;
    writeln!(
        out,
        "  omni --resume {LATEST_SESSION_REFERENCE} /status /diff /export notes.txt"
    )?;
    writeln!(out, "  omni agents")?;
    writeln!(out, "  omni mcp show my-server")?;
    writeln!(out, "  omni /skills")?;
    writeln!(out, "  omni doctor")?;
    ...
    writeln!(out, "  omni init")?;
    writeln!(out, "  omni export")?;
    writeln!(out, "  omni export conversation.md")?;
    Ok(())
}
...
mod tests {
    ...
    fn context_window_preflight_errors_render_recovery_steps() {
        ...
        assert!(
            rendered.contains("Resume compact   omni --resume session-issue-32 /compact"),
            "{rendered}"
        );
        assert!(
    ...
    fn default_permission_mode_uses_project_config_when_env_is_unset() {
        ...
        std::fs::create_dir_all(cwd.join(".omni")).expect("project config dir should exist");
        std::fs::create_dir_all(&config_home).expect("config home should exist");
        std::fs::write(
            cwd.join(".omni").join("settings.json"),
            r#"{"permissionMode":"acceptEdits"}"#,
        )
        .expect("project config should write");

        let original_config_home = std::env::var("OMNI_CONFIG_HOME").ok();
        let original_permission_mode = std::env::var("RUSTY_CLAUDE_PERMISSION_MODE").ok();
        std::env::set_var("OMNI_CONFIG_HOME", &config_home);
        std::env::remove_var("RUSTY_CLAUDE_PERMISSION_MODE");

        let resolved = with_current_dir(&cwd, super::default_permission_mode);

        match original_config_home {
            Some(value) => std::env::set_var("OMNI_CONFIG_HOME", value),
            None => std::env::remove_var("OMNI_CONFIG_HOME"),
        }
    ...
    fn status_degrades_gracefully_on_malformed_mcp_config_143() {
        // #143: previously `omni status` hard-failed on any config parse error,
        // taking down the entire health surface for one malformed MCP entry.
        // `omni doctor` already degrades gracefully; this test locks `status`
...
"#,
        ...
        .expect("write malformed .omni.json");
    ...
    fn state_error_surfaces_actionable_worker_commands_139() {
        // #139: the error for missing `.omni/worker-state.json` must name
        // the concrete commands that produce worker state, otherwise omnis
        ...
        assert!(
            message.contains("omni prompt"),
            "error should name `omni prompt <text>` as a producer: {message}"
        );
        assert!(
        ...
        // so omnis can discover the contract without hitting the error first.
        ...
        assert!(
            state_help.contains("omni prompt"),
            "state help must name `omni prompt <text>` as a producer: {state_help}"
        );
    }
    ...
    fn parses_direct_agents_mcp_and_skills_slash_commands() {
        ...
        assert!(error.contains("omni --resume SESSION.jsonl /status"));
    }
    ...
    fn punctuation_bearing_single_token_still_dispatches_to_prompt() {
        ...
        // doesn't pick up a stale .omni/settings.json from other tests that
    ...
    fn rejects_unknown_options_with_helpful_guidance() {
        ...
        assert!(error.contains("omni --help"));
    }
    ...
    fn repl_help_includes_shared_commands_and_exit() {
        ...
        assert!(help.contains("Auto-save            .omni/sessions/<session-id>.jsonl"));
    ...
    fn resolve_repl_model_falls_back_to_anthropic_model_env_when_default() {
        ...
        std::env::remove_var("ANTHROPIC_MODEL");
        std::env::remove_var("OMNI_CONFIG_HOME");
        fs::remove_dir_all(root).expect("cleanup temp dir");
    }
    ...
    fn init_help_mentions_direct_subcommand() {
        ...
        assert!(help.contains("omni help"));
        assert!(help.contains("omni version"));
        assert!(help.contains("omni status"));
        assert!(help.contains("omni sandbox"));
        assert!(help.contains("omni init"));
        assert!(help.contains("omni acp [serve]"));
        assert!(help.contains("omni agents"));
        assert!(help.contains("omni mcp"));
        assert!(help.contains("omni skills"));
        assert!(help.contains("omni /skills"));
        assert!(help.contains("visva/omni-agent"));
        assert!(help.contains("cargo install omni-agent"));
        assert!(!help.contains("omni login"));
        assert!(!help.contains("omni logout"));
    }
...
UU conflicted.rs",
    ...
    fn help_mentions_jsonl_resume_examples() {
        ...
        print_help_to(&mut help).expect("help should render");
        let help = String::from_utf8(help).expect("help should be utf8");
        assert!(help.contains("omni --resume [SESSION.jsonl|session-id|latest]"));
        assert!(help.contains("Use `latest` with --resume, /resume, or /session switch"));
        assert!(help.contains("omni --resume latest"));
        assert!(help.contains("omni --resume latest /status /diff /export notes.txt"));
    }
    ...
    fn managed_sessions_default_to_jsonl_and_resolve_legacy_json() {
        ...
        let legacy_path = workspace.join(".omni/sessions/legacy.json");
    ...
    fn load_session_reference_rejects_workspace_mismatch() {
        ...
        let session_path = workspace_a.join(".omni/sessions/legacy-cross.jsonl");
        std::fs::create_dir_all(
            session_path
                .parent()
    ...
    fn resume_usage_mentions_latest_shortcut() {
        ...
        assert!(usage.contains(".omni/sessions/<session-id>.jsonl"));
    ...
    fn temp_workspace(label: &str) -> PathBuf {
        let nanos = std::time::SystemTime::now()
            ...
            .as_nanos();
        std::env::temp_dir().join(format!("omni-cli-{label}-{nanos}"))
    }
...
mod dump_manifests_tests {
    ...
    fn dump_manifests_uses_explicit_manifest_dir() {
        let root = std::env::temp_dir().join(format!(
            "omni_test_explicit_manifest_dir_{}",
            std::process::id()
        ));
        let workspace = root.join("workspace");
