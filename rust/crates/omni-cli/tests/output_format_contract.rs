...
fn agents_command_emits_structured_agent_entries_when_requested() {
    ...
    assert_eq!(parsed["agents"][0]["source"]["id"], "project_omni");
    ...
    assert_eq!(parsed["agents"][2]["shadowed_by"]["id"], "project_omni");
}
...
fn doctor_and_resume_status_emit_json_when_requested() {
    ...
    assert_eq!(
        ...
        "https://github.com/visva/omni-agent"
    );
    assert_eq!(
        install_source["deprecated_install"],
        "cargo install omni-agent"
    );
...
fn assert_json_command_with_env(current_dir: &Path, args: &[&str], envs: &[(&str, &str)]) -> Value {
    let output = run_omni(current_dir, args, envs);
    assert!(
...
fn run_omni(current_dir: &Path, args: &[&str], envs: &[(&str, &str)]) -> Output {
...
fn write_upstream_fixture(root: &Path) -> PathBuf {
    let upstream = root.join("omni-agent");
...
fn unique_temp_dir(label: &str) -> PathBuf {
    ...
    std::env::temp_dir().join(format!(
        "omni-output-format-{label}-{}-{millis}-{counter}",
        std::process::id()
    ))
}
