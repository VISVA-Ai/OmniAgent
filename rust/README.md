# Omni Agent — Rust Implementation

A high-performance Rust rewrite of the Omni Agent CLI agent harness. Built for speed, safety, and native tool execution.
...
cargo run -p omni-cli -- --model claude-opus-4-6

# One-shot prompt
cargo run -p omni-cli -- prompt "explain this codebase"

# JSON output for automation
cargo run -p omni-cli -- --output-format json prompt "summarize src/main.rs"
```
...
- `crates/omni-cli/tests/mock_parity_harness.rs` — clean-env CLI harness
...
| Config file hierarchy (`.omni.json` + merged config sections) | ✅ |
| Permission system | ✅ |
...
`omni acp` is a local discoverability surface for editor-first users: it reports the current ACP/Zed status without starting the runtime. As of April 16, 2026, omni-agent does **not** ship an ACP/Zed daemon entrypoint yet, and `omni acp serve` is only a status alias until the real protocol surface lands.
...
Notable omni-first surfaces now available directly in slash form:
...
See [`../USAGE.md`](../USAGE.md) for usage examples and run `cargo run -p omni-cli -- --help` for the live canonical command list.
...
- **omni-cli** — REPL, one-shot prompt, direct CLI subcommands, streaming display, tool call rendering, CLI argument parsing
