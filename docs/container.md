# Container-first omni-agent workflows
...
- `rust/crates/omni-cli/src/main.rs` exposes that state through the `omni sandbox` / `cargo run -p omni-cli -- sandbox` report.
...
```bash
docker build -t omni-agent-dev -f Containerfile .
```

### Podman

```bash
podman build -t omni-agent-dev -f Containerfile .
```
...
docker run --rm -it \
  -v "$PWD":/workspace \
  -e CARGO_TARGET_DIR=/tmp/omni-target \
  -w /workspace/rust \
  omni-agent-dev \
...
If you want to run `omni` against a second checkout while keeping `omni-agent` itself mounted read-write:
...
```bash
cargo run -p omni-cli -- prompt "summarize /repo"
```
...
- Running with `CARGO_TARGET_DIR=/tmp/omni-target` avoids leaving container-owned `target/` artifacts in your bind-mounted checkout.
